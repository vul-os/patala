...
- **All seven crates declare `license = "MIT"` while the repo ships MIT OR Apache-2.0.** The metadata is *narrower* than what is offered, so nothing is over-claimed and nobody is misled into relying on a grant they do not have — which is why this is recorded rather than fixed. It is still a mismatch with real consequences: licence scanners and SBOM generators reading crate metadata will not see the Apache-2.0 option, and Apache-2.0 is the one carrying an explicit patent grant. The ecosystem convention is `license = "MIT OR Apache-2.0"`, a one-line change per manifest that would broaden, never narrow, what is granted. Which licences are offered is the copyright holder's decision, not a drive-by consistency fix. See the "Open owner decision" section in `README.md`.
