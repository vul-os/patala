...
[Status](status.md). The full flow — including the wording to put in front of a
...
- [Overview](overview.md) — why this exists and what it deliberately isn't.
- [One core, every language](polyglot.md) — how one Rust definition of this
  trait becomes a Python, Go and HTTP surface without being rewritten.
- [Crypto rails](rails-crypto.md) and [Fiat rails](rails-fiat.md) — what each
  implementation of the trait actually does.
- [Choosing a mode](choosing-a-mode.md) — crate, binding, or sidecar.
- [Status](status.md) — what's tested and what's still unverified against a
  live network.
