...
//! method at all (see `patala-hyperswitch`'s free-standing
//! `webhook::verify_webhook_signature`, and this crate's `manual.rs` module
//! docs on the same point). This is exposed as a plain function outside the
//! trait for callers that want push (webhook) delivery instead of polling
//! [`crate::stripe::StripeRail::verify`].
//!
