#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded patala release artifact, fail-closed.
#
...
#     name as a regex — every "." in "patala_9.9.9_c-abi_linux_amd64.tar.gz" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.1.1 patala_0.1.1_c-abi_linux_amd64.tar.gz
#   scripts/verify.sh --tag v0.1.1 --attest patala_0.1.1_c-abi_linux_amd64.tar.gz
#   scripts/verify.sh --repo vul-os/patala --tag v0.1.1 --out ~/Downloads ASSET
...
#
# Several assets can be named in one run; every one of them must verify or the
# run exits non-zero:
#
#   scripts/verify.sh --tag v0.1.1 \
#     patala_0.1.1_source.zip \
#     patala_0.1.1_c-abi_linux_amd64.tar.gz \
#     patala_0.1.1_c-abi_darwin_arm64.tar.gz
#
...
# ────────────────────────
# The C ABI bundles carry a shared library. A verified digest says these are
# the bytes vul-os/patala's release workflow built and published — it says
# nothing about whether you should run them, and it is not a substitute for
# `patala_abi_check()`, which is the library telling you at load time that it
# is the version your bindings were generated against (see include/patala.h).
# =============================================================================

set -euo pipefail

# ── Things a copying repo changes ────────────────────────────────────────────
DEFAULT_REPO="vul-os/patala"
MANIFEST="SHA256SUMS"
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/patala-verify.XXXXXX")"
...
ART   = "patala_9.9.9_c-abi_linux_amd64.tar.gz"
GOOD  = b"patala-c-abi-bundle-bytes\n" * 64
...
trap_sums = (line(ART + ".sig", OTHER) +
             line("patala_9X9X9_c-abi_linux_amd64.tar.gz", OTHER)).encode()
...
run_selftest() {
  ...
  local art="patala_9.9.9_c-abi_linux_amd64.tar.gz"
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/patala --base-url "$base/good" "$art"
