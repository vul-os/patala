// patala DIRECT mode on Deno — libpatala_ffi inside this process, over the C
...
//
// `--allow-ffi` and nothing else. No --allow-net, no --allow-read, no
// --allow-env — and unlike the sibling products' equivalents, this example is
// not merely declining to ask for network permission it could bypass anyway:
// the mock rail has no code path to a socket at all. `--allow-ffi` is
// effectively "allow anything" (Deno's own docs say so), so the flag list is
// evidence about intent, not a guarantee. The guarantee here is the rail.
//
...
import { abiCheck, abiVersion, Rail, type Receipt, resolveLibrary } from "../mod.ts";
...
// ===========================================================================
{
  ...
  // thing.
  console.log(
    `caps        ${caps.class}, reversible ${caps.reversible}, holds_funds ${caps.holds_funds}`,
  );
  console.log(
    `            currencies ${caps.currencies.join(", ")}, settlement ${JSON.stringify(caps.settlement)}\n`,
  );
  ...
  for (const dest of ["mock:wallet:alice", "mock:program:vault", "stellar:wallet:alice", "nonsense"]) {
    ...
    console.log(
      `destination ${dest.padEnd(21)} ${status} refusal=${v.is_refusal} confirm=${v.human_must_confirm}`,
    );
  }
...
// ===========================================================================
{
  ...
  console.log(
    `opaque      ${v.status}, refusal=${v.is_refusal} confirm=${v.human_must_confirm} — a fiat rail's honest answer`,
  );
}
...
// ===========================================================================
// 6. Off the isolate's thread
// ===========================================================================
// callAsync's symbol is declared `nonblocking: true`, so Deno runs it on a
// blocking-task thread and this isolate stays free.
//
// It buys nothing on the mock rail, and the honest measurement below says so:
// the call finishes in microseconds, so a 1 ms timer racing it ticks 0 times
// either way. The point is structural, not measurable here — a real rail's
// `charge` is a network round trip to Solana, Stellar or a processor, and that
// is the one you do not want on the isolate's thread.
{
  ...
  clearInterval(timer);
  console.log(`async       charged ${r.amount_minor} ${r.currency} off-thread in ${took.toFixed(2)} ms`);
  console.log(`            the isolate ticked ${ticks}x meanwhile — 0 because there was no time to tick in`);
