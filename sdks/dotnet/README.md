...
2. **A C ABI, so JSON strings rather than types.** No `IAsyncEnumerable`, and
   ...
   The Kotlin SDK faced the equivalent question with UniFFI's *own* first-party
   generator — which this workspace runs as its own binary, so it cannot drift
   — and took the generated route:
   [Reproducible generation](../kotlin/README.md#reproducible-generation) is
   what makes that safe there and is exactly what is missing here.
