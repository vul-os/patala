...
So Java takes the plain C ABI. **Kotlin does not any more**: the blocker that
kept [`sdks/kotlin`](../kotlin/README.md) on this binding — a UniFFI Kotlin
codegen bug on error variants with a field named `message` — was removed by
renaming that field to `detail`, and that SDK is now generated UniFFI with real
`PayRequest`/`Receipt`/`DestinationVerdict` types. Java cannot follow it, for
the reason above: there is no Java backend to generate from. This page's
JSON-string surface is a consequence of that, not a preference.
...
1. **A C ABI, so JSON strings rather than types.** The direct path hands you
   documents. [The Kotlin SDK](../kotlin/README.md) gets typed records instead,
   from generated UniFFI — a road that is open to it and closed to Java, which
   has no UniFFI backend. The price Kotlin pays for it is JNA, i.e. the second
   native artifact the next section refuses.
